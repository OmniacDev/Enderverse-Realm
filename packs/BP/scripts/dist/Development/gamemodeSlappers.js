                  ...
                  .then(responseData => {
                    if (!responseData.canceled) {
                      eventData.target.setDynamicProperty(
                        `slapperGamemode`,
                        slapperGamemodes[responseData.formValues[0]]
                      )

                      eventData.target.nameTag = /** @type {string} */ responseData.formValues[1]
                    }
                  })
                break
              case 1:
              ...
              case 0:
                new ModalFormData()
                  ...
                  .dropdown(
                    ...
                    slapperGamemodes,
                    slapperGamemodes.indexOf(eventData.target.getDynamicProperty(`slapperGamemode`))
                  )
  ...
  if (eventData.damagingEntity.typeId === 'minecraft:player' && eventData.hitEntity.hasTag('slapper')) {
    switch (eventData.hitEntity.getDynamicProperty(`slapperGamemode`)) {
      ...
      case `none`:
        break
      case `kitpvp`:
        KitPVP_Teleport(/** @type {Player} */ eventData.damagingEntity)
        break
      case `adventure`:
        Adventure_Teleport(/** @type {Player} */ eventData.damagingEntity)
        break
      case `arcade`:
        Arcade_Teleport(/** @type {Player} */ eventData.damagingEntity)
        break
      case `skywars`:
        Skywars_Teleport(/** @type {Player} */ eventData.damagingEntity)
        break
      case `bridge`:
        Bridge_Teleport(/** @type {Player} */ eventData.damagingEntity)
        break
      default:
        break
    }
  ...
  if (eventData.id === 'ec:leaveGame') {
    if (eventData.sourceEntity) {
      ...
      if (entity.typeId === 'minecraft:player') {
        ...
        if (gameID) {
          console.warn(
            gameManager.lobbies.find(data => data.id === gameID)?.tryDisconnectPlayer(/** @type {Player} */ entity)
          )
        }
      }
    }
