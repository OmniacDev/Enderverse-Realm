...
world.afterEvents.entityHitEntity.subscribe(data => {
    if ((data.damagingEntity.typeId === "minecraft:player") && (data.hitEntity.typeId === "minecraft:npc") && (data.hitEntity.getDynamicProperty(config.properties.is_slapper) ?? false)) {
...
function EditSlapper(arg) {
    const gamemode_names = config.gamemodes.map(entry => { return entry.display_name })
    const selected_gamemode_index = config.gamemodes.findIndex(entry => { return entry.id === arg.slapper.getDynamicProperty(config.properties.gamemode_id)})
...
function DeactivateSlapper(arg) {
    const DeactivateConfirmationForm = new MessageFormData()
        ...
        .body("Are you sure you want to §cdeactivate§r this slapper? (This will remove all settigs and return the slapper to its default NPC state)")
        .button1("NO")
        .button2("YES")
...
function RunSlapper(arg) {
    ...
    if (gamemode_id !== undefined) {
        const gamemode = config.gamemodes.find(entry => { return entry.id === gamemode_id})
