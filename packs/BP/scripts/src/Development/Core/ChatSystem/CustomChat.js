...
world.beforeEvents.chatSend.subscribe(beforeEventData => {
    ...
    /** @type {string} */
    const commandPrefix = world.getDynamicProperty(`commandPrefix`)
    ...
    if (originalMessage.startsWith(commandPrefix)) {
        if (originalMessage.startsWith(`${commandPrefix}prefix `)) {
            ...
            if (originalMessage === commandPrefix) {
                beforeEventData.sender.sendMessage(`§c[!] Please enter a valid command. Use §e${commandPrefix}help§c to see a list of available commands.`)
            }
            else {
                beforeEventData.sender.sendMessage(`§c[!] Could not find command §7${originalMessage}§c. Please use §e${commandPrefix}help§c to see a list of available commands.`)
            }
        }
    }
    else {
        if (beforeEventData.sendToTargets === true) {
            beforeEventData.targets.forEach(target => {
                target.sendMessage(`§8[§7${beforeEventData.sender.getDynamicProperty(`chatRole`)}§8] §7${beforeEventData.sender.name} §8»§7 ${originalMessage}`);
            })
        }
        else {
            world.sendMessage(`§8[§7${beforeEventData.sender.getDynamicProperty(`chatRole`)}§8] §7${beforeEventData.sender.name} §8»§7 ${originalMessage}`)
        }
    }
})